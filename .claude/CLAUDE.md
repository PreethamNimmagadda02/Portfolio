...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-21 (commit 70e1ca6). Confidence: 100%.
...
### Architecture
repo is a high-performance motion-orchestration library that consumes raw UI state and user interaction events, processes them through a reactive animation pipeline, and outputs hardware-accelerated CSS transitions and frame-synced DOM updates. It serves as the core animation engine for the application, abstracting complex viewport-aware transitions and accessibility-compliant motion behaviors into a declarative component API.

### Hotspots (high churn — check `get_risk` before editing)
- `.repowise/.update.log` — 10 commits/90d (100.0th %ile)
- `.repowise/knowledge-graph.json` — 10 commits/90d (98.6th %ile)
- `src/app/globals.css` — 9 commits/90d (97.1th %ile)
- `src/components/Experience.tsx` — 10 commits/90d (95.7th %ile)
- `src/components/Hero.tsx` — 12 commits/90d (94.3th %ile)

### Code health
Three co-equal signals: defect risk 6.84/10 avg, hotspot health 5.0/10 (stable), worst `src/components/GitHubStats.tsx` at 3.09/10 · maintainability 9.33/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
