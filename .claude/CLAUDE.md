...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-18 (commit 16c6fc3). Confidence: 100%.
...
### Architecture
repo is a high-performance frontend orchestration engine that consumes raw UI state and viewport telemetry to drive reactive 3D scene rendering, motion transitions, and responsive component layouts. It ingests browser-native events and configuration schemas, processes them through a centralized state management pipeline, and outputs synchronized visual effects and interactive DOM elements.

### Hotspots (high churn — check `get_risk` before editing)
- `src/components/Hero.tsx` — 9 commits/90d (100.0th %ile)
- `src/components/Experience.tsx` — 8 commits/90d (97.9th %ile)
- `src/components/ParticleField.tsx` — 3 commits/90d (95.7th %ile)
- `src/components/Projects.tsx` — 8 commits/90d (93.6th %ile)
- `src/components/GitHubStats.tsx` — 11 commits/90d (91.5th %ile)

### Code health
Three co-equal signals: defect risk 6.79/10 avg, hotspot health 5.12/10 (stable), worst `src/components/Experience.tsx` at 4.25/10 · maintainability 9.29/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
