...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-19 (commit 17d7439). Confidence: 100%.
...
### Architecture
repo is a high-performance frontend component library that transforms raw UI state and user interaction events into fluid, motion-optimized web interfaces by orchestrating Framer Motion animations, viewport-aware hooks, and performance-monitored rendering pipelines. The repository consumes React component definitions and motion configurations, processes them through a reactive state management layer, and outputs accessible, interactive UI elements designed for low-latency user experiences.

### Hotspots (high churn — check `get_risk` before editing)
- `src/app/globals.css` — 9 commits/90d (100.0th %ile)
- `src/components/Experience.tsx` — 10 commits/90d (98.5th %ile)
- `src/components/Hero.tsx` — 11 commits/90d (97.0th %ile)
- `.repowise/knowledge-graph.json` — 6 commits/90d (95.5th %ile)
- `src/components/Projects.tsx` — 10 commits/90d (94.0th %ile)

### Code health
Three co-equal signals: defect risk 6.85/10 avg, hotspot health 5.5/10 (stable), worst `src/components/GitHubStats.tsx` at 3.09/10 · maintainability 9.29/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
