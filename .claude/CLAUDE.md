...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-18 (commit 8796edd). Confidence: 100%.
...
### Architecture
repo is a high-performance frontend framework that consumes raw UI component definitions and viewport state data, processes them through a reactive motion-orchestration pipeline, and renders interactive, hardware-accelerated 3D scenes and responsive web interfaces. The repository serves as a centralized engine for managing complex UI transitions, performance monitoring, and cross-device layout synchronization.

### Hotspots (high churn — check `get_risk` before editing)
- `src/components/Hero.tsx` — 9 commits/90d (97.7th %ile)
- `src/components/Experience.tsx` — 8 commits/90d (95.3th %ile)
- `src/components/ParticleField.tsx` — 3 commits/90d (93.0th %ile)
- `src/components/Projects.tsx` — 8 commits/90d (90.7th %ile)
- `src/components/GitHubStats.tsx` — 11 commits/90d (88.4th %ile)

### Code health
Three co-equal signals: defect risk 5.66/10 avg, hotspot health 3.17/10 (stable), worst `src/components/SkillsMarquee.tsx` at 1.61/10 · maintainability 9.29/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `src/components/Hero.tsx` — untested hotspot — impact −2.0
- `src/components/Navbar.tsx` — untested hotspot — impact −2.0
- `src/components/Projects.tsx` — untested hotspot — impact −2.0
- `src/components/SkillsMarquee.tsx` — untested hotspot — impact −2.0
- `src/components/Experience.tsx` — change entropy — impact −1.1
